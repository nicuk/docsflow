...
-- Based on schema analysis - this is likely a foreign key constraint issue, not RLS

-- =============================================================================
-- STEP 1: Check if user exists in public.users (THIS IS THE LIKELY ISSUE)
-- =============================================================================

SELECT 
    'USER EXISTENCE CHECK' as check_type,
    EXISTS(SELECT 1 FROM users WHERE id = 'cc362aeb-bf97-4260-9dfb-bb172c9c202') as user_exists_in_public_users,
    EXISTS(SELECT 1 FROM auth.users WHERE id = 'cc362aeb-bf97-4260-9dfb-bb172c9c202') as user_exists_in_auth_users;
...
SELECT 
    ...
    name
FROM tenants 
WHERE id = '122928f6-f34e-484b-9a69-7e1f25caf45c' OR subdomain = 'bitto';
...
SELECT 
    au.id,
    '122928f6-f34e-484b-9a69-7e1f25caf45c' as tenant_id,
    au.email,
