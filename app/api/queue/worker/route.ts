...
async function processDocumentContent(
  ...
  if (useMultimodal) {
    ...
    try {
      ...
      // 🆕 NEW: Skip old embedding generation - Pinecone workflow handles this
      // Embeddings will be generated by ingestWorkflow using OpenRouter
      console.log(`[JOB ${job.id}] Skipping old embedding generation (handled by Pinecone workflow)`);
  ...
  // 🆕 NEW: Ingest chunks to Pinecone using new workflow
  if (parsedDocument.chunks.length > 0) {
    console.log(`💾 [JOB ${job.id}] Ingesting ${parsedDocument.chunks.length} chunks to Pinecone...`);
    
    try {
      // Import new ingest workflow
      const { ingestWorkflow } = await import('@/lib/rag');
      ...
      // Ingest to Pinecone (generates embeddings + upserts)
      const ingestResult = await ingestWorkflow({
      ...
      console.log(`   - Chunks processed: ${ingestResult.chunksProcessed}`);
      console.log(`   - Vectors upserted: ${ingestResult.vectorsUpserted}`);
      console.log(`   - Duration: ${ingestResult.metrics.duration}ms`);
      console.log(`   - Avg time per chunk: ${ingestResult.metrics.avgEmbeddingTime.toFixed(0)}ms`);
