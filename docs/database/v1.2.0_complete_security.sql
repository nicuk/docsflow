-- SECURE SCHEMA FIXES FOR PRODUCTION DEPLOYMENT (COMPLETE)
...
DECLARE
    func_record RECORD;
BEGIN
    -- Find all similarity_search functions and drop them
    FOR func_record IN 
        SELECT p.proname, pg_get_function_identity_arguments(p.oid) as args
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname = 'similarity_search' AND n.nspname = 'public'
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS public.similarity_search(%s)', func_record.args);
        RAISE NOTICE 'Dropped function similarity_search(%)', func_record.args;
    END LOOP;
    
    -- Also try common variations that might exist
    DROP FUNCTION IF EXISTS public.similarity_search(vector, float, int, text, int);
    DROP FUNCTION IF EXISTS public.similarity_search(vector, float, int, uuid, int);
    DROP FUNCTION IF EXISTS public.similarity_search(vector, text, int);
    DROP FUNCTION IF EXISTS public.similarity_search(vector);
    
    RAISE NOTICE 'All similarity_search functions have been dropped';
END;
$$;

-- 4. Fix inconsistent tenant_id types (standardize to UUID)
...
-- 5. Fix embedding column type (only if needed)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 6. Add missing foreign key constraints (only if they don't exist)
DO $$
BEGIN
...
-- 7. Add critical indexes for performance (IF NOT EXISTS)
...
-- 8. Enable Row Level Security on all tables (safe to run multiple times)
...
-- 9. Recreate RLS policies with correct UUID types
...
-- 10. CREATE THE SECURE vector similarity search function (FIXES SECURITY WARNING)
-- This is now safe to create since we dropped all existing versions
...
-- 11. Security permissions for function (safe to run multiple times)
...
BEGIN
  ...
  -- Check if function exists and count how many
  SELECT EXISTS (
  ...
  RAISE NOTICE 'similarity_search function count: %', function_count;
  ...
  IF function_count != 1 THEN
    RAISE EXCEPTION 'FAILED: Expected exactly 1 similarity_search function, found: %', function_count;
  END IF;
  ...
  RAISE NOTICE 'Function signature: similarity_search(vector(768), float, int, uuid, int)';
END;
$$; 