-- Fix onboarding function signature mismatch
-- The function exists with wrong parameters, need to recreate with correct signature

-- Drop the existing function with wrong signature
DROP FUNCTION IF EXISTS complete_onboarding_atomic(JSONB, UUID);
DROP FUNCTION IF EXISTS complete_onboarding_atomic(UUID, TEXT);

-- Create atomic onboarding function with correct signature
...
FROM pg_proc 
WHERE proname = 'complete_onboarding_atomic';
